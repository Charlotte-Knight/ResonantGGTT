...
# def halveDf(df, score, cr_sr, half):
#   median_score = df[df.SR==cr_sr][score].median()
#   if half == 0:
#     df = df[(df.SR!=cr_sr)|(df[score]<median_score)]
#   elif half == 1:
#     df = df[(df.SR!=cr_sr)|(df[score]>median_score)]
#   else:
...
def doFits(df, df_inverted, optim_result, half=None, plot=True, save_dir=None):
  ...
  if half != None:
    ...
    # if half == 0:
    #   split_point = df[df.SR==cr_sr][score].quantile(0.25)
    # else:
    #   split_point = df[df.SR==cr_sr][score].quantile(0.75)
  ...
  C_N_peak, C_N_peak_err, C_popt, C_perr = fitFunction(C, save_dir+"C_fit.png", "Region C", plot=plot)
  ...
  C_yield, C_yield_err = C_N_peak, C_N_peak_err
...
def correlationCheck(df, df_inverted, optim_results, only_90=False):
  ...
  for i, res in enumerate(optim_results):
    ...
    info0s[res["sig_proc"]] = doFits(df, df_inverted, res, half=0, plot=False, save_dir=f"RelicDYEstimation/{res['sig_proc']}/LowerHalf/")
    info1s[res["sig_proc"]] = doFits(df, df_inverted, res, half=1, plot=False, save_dir=f"RelicDYEstimation/{res['sig_proc']}/UpperHalf/")
    infos_whole[res["sig_proc"]] = doFits(df, df_inverted, res, plot=False, save_dir=f"RelicDYEstimation/{res['sig_proc']}/Whole/")
...
# load in inverted electron veto df and invert pixel veto
...
#df_inverted = df_inverted[df_inverted.process_id.isin([proc_dict_inverted[proc+"invEveto"] for proc in ["ZZ", "WW", "WZ"]])]
...
#df_inverted = df_inverted[(df_inverted.Diphoton_mass>91-1.08)&(df_inverted.Diphoton_mass<91+1.08)]
