...
NMSSM_max_allowed_Y_gg = pd.DataFrame({"MX":[410,405,413,500,500,500,600,600,600,700,700,700,  300,300,300, 800, 800, 800],
                                       "MY":[70,100,200,70,100,200,70,100,200,70,100,200,   70,100,200, 70,100,200], 
                                       "limit":[4.08,8.85,4.06,0.916,1.62,1.26,0.214,0.365,0.370,0.0580,0.103,0.120,   4.08,8.85,4.06, 0.0580,0.103,0.120,]})                                       
...
def plotLimits2D(masses, limits, ylabel, savename, observed=False):
  ...
  spline = spi.LinearNDInterpolator(NMSSM_max_allowed_Y_gg[["MX","MY"]], NMSSM_max_allowed_Y_gg.limit, fill_value=0)
  max_allowed_values = [spline(m[0], m[1]) for m in masses]
  ...
  max_allowed_values_interp = np.array([spline(m[0], m[1]) for m in interp_masses])
  ...
  plt.hist2d(interp_masses[:,0], interp_masses[:,1], [mx_edges, my_edges], weights=max_allowed_values_interp, norm=matplotlib.colors.LogNorm())
  ...
  s = interp_limits < max_allowed_values_interp
  plt.scatter(interp_masses[s,0], interp_masses[s,1], marker='x', color="r", label="Limit below maximally allowed in NMSSM") 
  #plt.scatter(interp_masses[~s,0], interp_masses[~s,1], marker='x', color="b", label="Limit below maximally allowed in NMSSM") 

  #plt.fill_between([250,650],[65, 65],[130,130],facecolor="none",hatch="x",edgecolor="red", label="Limit below maximally\nallowed in NMSSM")
  ...
  # plt.scatter(masses[~s,0], masses[~s,1], marker='x', color="b", label="Limit below maximally allowed in NMSSM") 
...
def plotSignificance2D(masses, limits, ylabel, savename):
  ...
  mlabel = mplhep.cms.label(llabel="", data=True, lumi=common.tot_lumi, loc=0)
  plt.savefig(savename+"_paper.png")
  plt.savefig(savename+"_paper.pdf")
  ...
  max_sig_str = "Max. sig. of %.1f at\n"%max_sig + r"$(m_X, m_Y) = (%d, %d)$"%(max_mass[0], max_mass[1])
  plt.text(0.55, 0.85, max_sig_str, transform=plt.gca().transAxes, fontsize=24, color="black")

  mlabel[1].set_text("Supplementary")
  plt.savefig(savename+"_legend_supplementary.png")
  plt.savefig(savename+"_legend_supplementary.pdf")

  plt.clf()

def outputLimits(masses, limits, outdir):
  df = pd.DataFrame({"MX": masses[:,0], "MY": masses[:,1], "exp": limits[2], "obs": limits[5],
