...
def createDefaultConfig(data, bkg, sig, args):
  ...
  for column in columns_to_plot:
    ...
    if d.dtype == int:
      ...
      low = min([d.quantile(0.05), b.quantile(0.05), s.quantile(0.01)])
      high = max([d.quantile(0.95), b.quantile(0.95), s.quantile(0.99)])
...
def plot_feature(data, bkg, sig, proc_dict, sig_procs, column, nbins, feature_range, save_path, no_legend=False, only_legend=False, limits=None, delete_zeros=False):
  ...
  if not only_legend:
    ...
    plt.subplots_adjust(hspace=0.1,left=0.15,right=0.95)
  else:
    f, axs = plt.subplots(2, sharex=True, gridspec_kw={'height_ratios': [10, 1]})
  ...
  for sig_proc in sig_procs:
    ...
    axs[0].hist(edges[:-1], edges, weights=sig_hist*sig_sf, label=getSigLabel(sig_proc, sig_sf), histtype='step', lw=4, path_effects=[pe.Stroke(linewidth=6, foreground='w'), pe.Normal()], zorder=3) #signal
  ...
  if only_legend: 
    ...
    legend = plt.legend(handles, labels, ncol=3, loc="center", markerfirst=False, frameon=True, framealpha=1.0)
    ...
    plt.savefig("%slegend.pdf"%save_path)
    return
  ...
  def thousand_formatter(x, pos):
    return f'{x/100:.0f}'  # Format with 'k' suffix
  # Apply the formatter to the y-axis
  axs[0].yaxis.set_major_formatter(mticker.FuncFormatter(thousand_formatter))
  axs[0].set_ylabel(r"Events ($\times 10^2$)")
